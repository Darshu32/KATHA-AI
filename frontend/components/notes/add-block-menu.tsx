  ...
  return (
    <div className="relative flex justify-center py-0.5 opacity-0 hover:opacity-100 transition-opacity">
      <button
        ...
        className="p-0.5 text-ink-mute hover:text-ink-deep hover:bg-paper-soft rounded transition-colors"
        title="Add block"
      >
        <Plus size={12} />
      </button>
      {open && (
        <div className="absolute top-5 left-1/2 -translate-x-1/2 z-20 bg-paper border border-hairline rounded-lg shadow-card py-1 w-36">
          {OPTIONS.map((opt) => (
            <button
              key={opt.type}
              onClick={() => {
              ...
              className="w-full text-left px-3 py-1.5 hover:bg-paper-soft transition-colors"
            >
              <span className="text-xs font-medium text-ink-deep">{opt.label}</span>
              <span className="text-[10px] text-ink-mute ml-1.5">{opt.desc}</span>
            </button>
          ))}
        </div>
      )}
